...
#define MPU_MOVE_DELAY_US 500000      // 500ms entre movimentos (aumentado)
#define MPU_TILT_THRESHOLD 2500       // Reduzido ainda mais para melhor sensibilidade
#define DISPLAY_UPDATE_MS 200         // Atualiza display a cada 200ms
...
void jogo_loop()
{
    ...
    ultimo_envio_nuvem = get_absolute_time();

    char Direcao_str[20] = "Parado", Posicao_str[20], Nivel_str[20];
    ...
    while (true)
    {
        ...
        // Detecta movimento baseado no MPU6050
        if (absolute_time_diff_us(ultimo_movimento_mpu, agora) > MPU_MOVE_DELAY_US)
        {
            int dx = 0, dy = 0;
            movimento_detectado = false;
            
            // IMPORTANTE: Inverte os eixos se necessário
            // Testa primeiro com Y para esquerda/direita
            if (accel_data.y > MPU_TILT_THRESHOLD) { 
                dx = 1; 
                strcpy(Direcao_str, "Direita"); 
                movimento_detectado = true;
            }
            else if (accel_data.y < -MPU_TILT_THRESHOLD) { 
                dx = -1; 
                strcpy(Direcao_str, "Esquerda"); 
                movimento_detectado = true;
            }
            // X para frente/trás
            else if (accel_data.x > MPU_TILT_THRESHOLD) { 
                dy = 1; 
                strcpy(Direcao_str, "Baixo"); 
                movimento_detectado = true;
            }
            else if (accel_data.x < -MPU_TILT_THRESHOLD) { 
                dy = -1; 
                strcpy(Direcao_str, "Cima"); 
                movimento_detectado = true;
            }
            ...
            if (movimento_detectado)
            {
                ...
                printf("Movimento: dx=%d, dy=%d, Jogador: (%d,%d)\n", dx, dy, jogador_x, jogador_y);
            }
        }

        // Atualiza display apenas a cada DISPLAY_UPDATE_MS
        if (absolute_time_diff_us(ultimo_update_display, agora) / 1000 > DISPLAY_UPDATE_MS)
        {
            sprintf(Nivel_str, "Nivel: %d", nivel);
            sprintf(Posicao_str, "Pos:(%d,%d)", jogador_x, jogador_y);
            
            char accel_info[20];
            sprintf(accel_info, "X:%d Y:%d", accel_data.x / 100, accel_data.y / 100);
            
            display_mensagem("LABIRINTO", Nivel_str, Posicao_str, Direcao_str, accel_info, "", "", "", true);
        ...
        if (absolute_time_diff_us(ultimo_envio_nuvem, agora) / 1000 > NUVEM_SEND_INTERVAL_MS)
        {
            ...
            ultimo_envio_nuvem = agora;
        }

        sleep_ms(50); // Loop mais rápido para melhor resposta
    }
}